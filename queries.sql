...
select AVG(weight_kg) as SUM from animals;
select name from animals where escape_attempts =(select max(escape_attempts) from animals);
select species,min(weight_kg) as min, max(weight_kg) as max from animals group by species;
select species,avg(escape_attempts) as AVG from animals where date_of_birth between '1990-01-01' and '2000-12-31'group by species;

--JOIN------------------------------------------------------------------------
select name from animals
join owners on animals.owner_id = owners.id and owners.full_name='Melody Pond';
...
select count(animals.name) as number, species.name from animals
join species on  animals.species_id= species.id group by species.name;

select owners.full_name, species.name from owners
...
select animals.name from animals
join owners on owners.id = animals.owner_id and owners.full_name='Dean Winchester' and animals.escape_attempts = 0;
